...
def initial_guesss(ode, x0 , target_time ,*args):
    """
    A function that give a approximate values for the initial values
    ...
    target_time : The end time starting from 0
    args : Any extra arguments .

    Returns
    -------
    Returns a numpy.array containing a approximate guess initial values
    for the limit cycle.
    """
    n = target_time*20
...
def shooting(ode, u0,*args , method='fsolve' ,phase_condition = lambda u0,ode, *args: ode(u0[:-1], 1, *args)[0] ,plot=False):
    ...
    u0 : numpy.array
        An initial guess at the initial values for the limit cycle.
    args : Any extra arguments .
    method : The root-finding method used ('fsolve' or 'newton')
    phase_condition : the phase condition equation (The default phase condition is dx/dt=0)
    plot: The statement of plot figure or not(if plot figure, let plot=True)
    Returns
    -------
...
def main():
    ...
    print(solve_ode(predator_prey, [1, 1], 500, 'rk4', a, b, d, n=500, plot='plot_x_y'))
    #By looking at the figure generated by the function above
    #Shows that it behave in a orbit
    # plot figure of x againist y with b= 0.45
    b = 0.45
    print(solve_ode(predator_prey, [1, 1], 100, 'rk4', a, b, d, n=100, plot='plot_x_y'))
    #By looking at the figure generated by the function above
    #Shows that it dont behave in a orbit and stoped at a point finally
    ...
    #In order, to isolate a periodic orbit the function used below is created to get the values of the cycle time
    #of a single periodic orbit and get a good inital starting condition to start with

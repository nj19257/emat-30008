...
def solve_ode( f , x , t_0 , t_end , deltat_max , Method  , *args, n=5 ,**kwargs):#plot=False,):
    """
    single Euler step function
        Parameter:
            f(function) = function used ( need to define more detail)
...
def error_depend_deltat_t( f ,x , t_0 , t_end ,h, Method , *args ,**kwargs): # plot=False , timer = False):
    ...
    for i in range(len(h)):
        x_list ,t =solve_ode(f, x, t_0 , t_end , h[i], Method,*args,**kwargs)
        if f == dxdt_equal_x:
...
solve_ode(d2xdt2_equals_minus_x, [1, 1], 0, 100, 0.001, 'euler',  plot=True , timer =True )
